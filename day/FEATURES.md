...
| 功能 | 代码位置 | 说明 |
|------|---------|------|
| 收藏系统 | `storage.js` - FavoritesManager | 完整实现，无 UI 入口 |
| 搜索历史 | `storage.js` - SearchHistoryManager | 完整实现，无 UI 入口 |
| 用户偏好设置 | `storage.js` - PreferencesManager | 完整实现，无 UI 入口 |
| 数据导出/导入 | `app.js` - exportUserData/importUserData | 完整实现，无 UI 入口 |
| 课程推荐 | `courses/index.js` - getRecommendedCourses | 完整实现，未调用 |
| 热门课程 | `courses/index.js` - getPopularCourses | 完整实现，未调用 |
| 平台筛选 | `courses/index.js` - filterCourses | 完整实现，无 UI 入口 |
| 视图切换 | `storage.js` - viewMode (grid/list) | 数据模型存在，无 UI |
